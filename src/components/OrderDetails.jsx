...
const OrderDetails = () => {
  ...
  const handleCompleteOrder = async (orderId) => {
    try {
      ...
      // Optionally, you might want to add a delay or check the status before deletion
  ...
  return (
    <div className="container mx-auto p-6">
      ...
      <ul className="space-y-4">
        {orders.length > 0 ? (
          orders.map(order => (
            <li key={order.id} className="border border-gray-300 rounded-lg p-4 bg-white shadow-md">
              <h2 className="text-2xl font-semibold text-gray-800">Order ID: {order.id}</h2>
              <p className="text-lg text-gray-700">Total Bill: ₹{order.totalBill}</p>
              <p className="text-sm text-gray-500">Timestamp: {new Date(order.timestamp.toDate()).toLocaleString()}</p>
              <h3 className="text-xl font-medium text-gray-800 mt-4">Items:</h3>
              ...
              <button
                ...
                className="mt-4 bg-red-500 text-white px-4 py-2 rounded transition-colors hover:bg-red-600"
              >
                Complete Order
              </button>
            </li>
